...
class Timeseries(pd.DataFrame):
    ...
    def from_file(cls,filename):
        ...
        with open(filename) as file:
            for i, line in enumerate(file):
                if i == 3: #3:
                    ...
                    logger.debug("Read coordinates from %s: x=%s y=%s z=%s", filename, x, y, z)
                    break
        ...
        # Keep NaNs for zeros (same behavior as u/u), but avoid runtime warnings.
        u_eq = np.divide(u, u, out=np.full_like(u, np.nan), where=u != 0)
        v_eq = np.divide(v, v, out=np.full_like(v, np.nan), where=v != 0)
    ...
    def get_wtref(self,wtref_path,filename,index=0,vscale=1.):
        ...
        try:
            ...
            logger.warning("ATTENTION: wtref-file not found at %s", wtreffile)
            self.wtref = None
            return
    ...
    def get_wind_comps(self,filename):
        ...
        logger.warning("Could not infer wind components from filename: %s. Falling back to header.", filename)
        with open(filename) as file:
            for i, line in enumerate(file):
                if i == 5:
        ...
        raise ValueError("Could not infer wind components from filename or file header: {}".format(filename))
