...
class Timeseries_nc(pd.DataFrame):
    ...
    def get_wtref(self,wtref_path,filename,index=0,vscale=1.):
        ...
        # Keep the historical path behavior (wtref_path + filename + suffix).
        wtreffile = wtref_path + filename + '_wtref.txt'
        try:
    ...
    def get_wind_comps(self,filename):
        ...
        raise ValueError("Could not infer wind components from filename: {}".format(filename))
